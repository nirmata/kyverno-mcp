...
The Kyverno MCP server includes three default policy sets that are automatically applied when scanning cluster resources:

- **Pod Security Standards**: Baseline security controls for container workloads
- **RBAC Best Practices**: Security policies for role-based access control
- **Kubernetes Best Practices**: General best practices for Kubernetes resources

### Using Custom Policy Sets

To use your own custom policies instead of the defaults, you can:

1. **Deploy custom policies** directly to your cluster using `kubectl apply`
2. **Use existing policies** from your cluster - the MCP server will scan against all policies present in the cluster
3. **Specify policy sets** when calling the MCP server tools by providing the `policySets` parameter

The MCP server will automatically discover and use any Kyverno policies deployed in your cluster, whether they are the default sets or your own custom policies.