...
func ApplyPolicies(s *server.MCPServer) {
	...
	applyPoliciesTool := mcp.NewTool(
		...
		mcp.WithString("policySets", mcp.Description("Policy set key: pod-security, rbac-best-practices, kubernetes-best-practices, all (default: all). Also accepts a git URL to apply policies from a remote repository.")),
